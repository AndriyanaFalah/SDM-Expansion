...
server = function(input, output){
  ...
  U = reactive({
    A  = A()
    V1 = matrix(1,nrow(dataframe()),1)
    U  = cbind(V1,A,W()%*%XTilde())  
    return(U)
  })
  
  output$warningU = renderText({
    if(length(input$variableX) == 0 | length(input$variableJ) == 0 | length(input$variableZ) == 0){
      return("Please fill in matrix of X, matrix of J, or matrix of Z")
    }
  })
  
  output$inputrowU= renderUI({
    if(length(input$variableX) == 0 | length(input$variableJ) == 0 | length(input$variableZ) == 0){
      return(NULL)
    }else{
      matrix = data.frame(U())
      div(style="display: inline-block;vertical-align:top;margin-top:30px;", textInput('numrowU', "Number of rows to display: "), span('from '), span(as.character(nrow(matrix))), span("rows"))
    }
  })
  output$inputcolU = renderUI({
    if(length(input$variableX) == 0 | length(input$variableJ) == 0 | length(input$variableZ) == 0){
      return(NULL)
    }else{
      matrix = data.frame(U())
      div(style="display: inline-block;vertical-align:top;", textInput('numcolU', "Number of columns to display: ",), span('from '), span(as.character(ncol(matrix))), span("columns"))
    }
  })
  
  output$matrixU = renderDT({
    if(length(input$variableX) != 0 & length(input$variableJ) != 0 & length(input$variableZ) != 0){
      matrix = data.frame(U())
      if(!isTruthy(input$numrowU) & !isTruthy(input$numcolU)){
        matrix = matrix[1:6,1:6]
      }else if(!isTruthy(input$numrowU)){
        matrix = matrix[1:6,1:input$numcolU]
      }else if(!isTruthy(input$numcolU)){
        matrix = matrix[1:input$numrowU,1:6]
      }else{
        matrix = matrix[1:input$numrowU,1:input$numcolU]
      }
      colnames(matrix) = paste(rep("[ ,", ncol(matrix)),as.character(c(1:ncol(matrix))),rep(']',ncol(matrix)))
      rownames(matrix) = paste(rep("[", nrow(matrix)),as.character(c(1:nrow(matrix))),rep(', ]',nrow(matrix)))
      matrix
  ...
  rhoSDM  = reactive({
    ...
    U = U()
    listEigv = eigen(W)
    eig = c(min(listEigv$values), max(listEigv$values))
    print(eig)
    print(min(eig))
    print(max(eig))
    rho = as.numeric(solve(t(Y)%*%t(W)%*%W%*%Y)%*%t(Y)%*%t(W)%*%Y)
    cat("rho:", rho, "\n")
    while((rho>=1/eig[2]) || (rho<=1/eig[1])){
      delta = (solve(t(U)%*%U)%*%(t(U)))%*%(Y-rho*(W%*%Y))
      rho = as.numeric(solve(t(W%*%Y)%*%(W%*%Y))%*%t(W%*%Y)%*%(Y-(U%*%delta)))
      cat("rho:", rho, "\n")
    }
    rhoDelta <<-list("rho" = rho, "delta" = delta)
    return(rhoDelta)
  })
  ...
  output$warningalpha = renderText({
  ...
  output$alpha = renderText({
    if(length(input$variableX) == 0 | length(input$variableJ) == 0 | length(input$variableZ) == 0 | length(input$variableY) == 0 | length(input$variableW) == 0){
      ...
      alpha = as.numeric(data.frame(rhoSDM()$delta)[1,1])
    }
  })
  ...
  output$inputrowb0 = renderUI({
    if(length(input$variableX) == 0 | length(input$variableJ) == 0 | length(input$variableZ) == 0 | length(input$variableY) == 0 | length(input$variableW) == 0){
      ...
      div(style="display: inline-block;vertical-align:top;margin-top:30px;", textInput('numrowb0', "Number of rows to display: "), span('from '), span(as.character(2*length(input$variableX))), span("rows"))
    }
  })
  output$b0 = renderDataTable({
    if(length(input$variableX) != 0 | length(input$variableJ) != 0 | length(input$variableZ) != 0 | length(input$variableY) != 0 | length(input$variableW) != 0){
      matrix = matrix(t(rhoSDM()$delta[2:(2*length(input$variableXTilde)+1),]),ncol=1)
      colnames(matrix) = "[ , 1]"
  ...
  output$warningtheta = renderText({
    if(length(input$variableX) == 0 | length(input$variableJ) == 0 | length(input$variableZ) == 0 | length(input$variableY) == 0 | length(input$variableW) == 0){
      return("Please fill in matrix of W, matrix of X, vector of Y, or matrix of Z")
    }
  })
  output$inputrowtheta = renderUI({
    if(length(input$variableX) == 0 | length(input$variableJ) == 0 | length(input$variableZ) == 0 | length(input$variableY) == 0 | length(input$variableW) == 0){
      return(NULL)
    }else{
      div(style="display: inline-block;vertical-align:top;margin-top:30px;", textInput('numrowTheta', "Jumlah baris yang ingin ditampilkan: "), span('dari '), span(as.character(length(input$variableX))), span("baris"))
    }
  })
  output$theta = renderDataTable({
    if(length(input$variableX) != 0 | length(input$variableJ) != 0 | length(input$variableZ) != 0 | length(input$variableY) != 0 | length(input$variableW) != 0){
      matrix = matrix(t(rhoSDM()$delta[(2*length(input$variableXTilde)+2):nrow(rhoSDM()$delta),]),ncol=1)
      print
      colnames(matrix) = "[ , 1]"
      if(!isTruthy(input$numrowTheta)){
        matrix
      }else{
        matrix = data.frame(matrix[1:input$numrowTheta,])
        colnames(matrix) = "[ , 1]"
        matrix
      }
    }else{
  ...
  betaWRTX = reactive({
    matrix = matrix(as.numeric(unlist(rhoSDM()$delta[2:(2*length(input$variableXTilde)+1),])), nrow = nrow(dataframe()), ncol = length(input$variableX), byrow = FALSE)
  })
  ...
  YHat = reactive({
    rho = as.numeric(rhoSDM()$rho)
    W = W()
    Y = Y()
    U = U()
    delta = rhoSDM()$delta
    yhat = (rho*(W%*%Y))+(U%*%delta)
    return(yhat)
  })
  ...
  output$downloadData <- downloadHandler(
    ...
    content = function(file) {
      if(input$downloaddata == "VY"){
        ...
        dataFrame = data.frame(matrix(t(rhoSDM()$delta[(2*length(input$variableXTilde)+2):nrow(rhoSDM()$delta),]),ncol=1))
      ...
      }else if (input$downloaddata == "MU"){
        dataFrame = data.frame(U())
      }
  ...
  output$showDownloadData = renderDT({
    if(length(input$variableX) != 0 | length(input$variableJ) != 0 | length(input$variableZ) != 0 | length(input$variableY) != 0 | length(input$variableW) != 0){
      if(input$downloaddata == "VY"){
        ...
        dataFrame = as.numeric(unlist(theta["theta"]))
      }else if(input$downloaddata == "VBY"){
        dataFrame = YHat()
      }else if(input$downloaddata == "VBE"){
        dataFrame = Error()
